...
2. **Suggest setting it up now** so the skill can be tested in the same job:
   - Run: `npx open-ohcode set-agent-llm-secret <KEY_NAME> <value>`
...
> ```
> npx open-ohcode set-agent-llm-secret SLACK_WEBHOOK_URL <your-url>
> ```
...
> 4. Test: run `post.sh "test message from open-ohcode"` and verify successful delivery. Fix any issues before committing.
