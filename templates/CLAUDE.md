...
This directory contains files that get copied into user projects when they run `npx open-ohcode init`. It is **not** where event handler logic, API routes, or core features live.
...
- **Next.js wiring**: `next.config.mjs`, `instrumentation.js`, catch-all route, middleware — thin re-exports from `open-ohcode/*`
...
- Anything that should be shared across all users via `npm update open-ohcode`
