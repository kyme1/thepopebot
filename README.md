...
**Free compute, built in** — Every GitHub account comes with free cloud computing time. open-ohcode uses that to run your agent. One task or a hundred in parallel — the compute is already included.
...
[![Star History Chart](https://api.star-history.com/svg?repos=stephengpope/open-ohcode&type=date&legend=top-left)](https://www.star-history.com/#stephengpope/open-ohcode&type=date&legend=top-left)
...
> npx open-ohcode set-var APP_URL https://your-new-url.ngrok.io
...
```bash
npm install open-ohcode@latest
```

**2. Scaffold and update templates**

```bash
npx open-ohcode init
```
...
```bash
git add -A && git commit -m "upgrade open-ohcode to vX.X.X"
git push
```

Pushing to `main` triggers the `rebuild-event-handler.yml` workflow on your server. It detects the version change, runs `open-ohcode init`, updates `THEPOPEBOT_VERSION` in the server's `.env`, pulls the new Docker image, restarts the container, rebuilds `.next`, and reloads PM2 — no manual `docker compose` needed.
...
When you ran `open-ohcode init` the first time, it scaffolded a project folder with two kinds of files:
...
To view differences:  npx open-ohcode diff <file>
To reset to default:  npx open-ohcode reset <file>
```

You can review at your own pace:

```bash
npx open-ohcode diff config/CRONS.json    # see what changed
npx open-ohcode reset config/CRONS.json   # accept the new template
```
...
```bash
npx open-ohcode init --no-managed
```
...
All commands are run via `npx open-ohcode <command>` (or the `npm run` shortcuts where noted).
...
The `templates/` directory contains files scaffolded into user projects by `open-ohcode init`. Two naming conventions handle files that npm or AI tools would otherwise misinterpret:
...
```bash
npx open-ohcode set-var LLM_PROVIDER openai
npx open-ohcode set-var LLM_MODEL gpt-4o
```
...
```bash
npx open-ohcode set-var LLM_PROVIDER custom
npx open-ohcode set-var LLM_MODEL deepseek-chat
npx open-ohcode set-var OPENAI_BASE_URL https://api.deepseek.com/v1
```

Then set the API key as a GitHub secret:

```bash
npx open-ohcode set-agent-secret CUSTOM_API_KEY sk-...
```
...
```bash
npx open-ohcode set-var RUNS_ON self-hosted
npx open-ohcode set-var LLM_PROVIDER custom
npx open-ohcode set-var LLM_MODEL qwen3:8b
npx open-ohcode set-var OPENAI_BASE_URL http://host.docker.internal:11434/v1
```
