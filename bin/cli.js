...
function printUsage() {
  console.log(`
Usage: open-ohcode <command>

Commands:
  init                              Scaffold a new open-ohcode project
...
async function init() {
  ...
  // Guard: warn if the directory is not empty (unless it's an existing open-ohcode project)
  ...
  if (entries.length > 0) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (deps.open-ohcode || devDeps.open-ohcode) {
  ...
  console.log('\nScaffolding open-ohcode project...\n');
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    const open-ohcodeDep = version.includes('-') ? version : '^1.0.0';
    const pkg = {
      name: dirName,
      private: true,
      scripts: {
        ...
        build: 'next build',
        start: 'next start',
        setup: 'open-ohcode setup',
        'setup-telegram': 'open-ohcode setup-telegram',
        'reset-auth': 'open-ohcode reset-auth',
      },
      dependencies: {
        open-ohcode: open-ohcodeDep,
        next: '^15.5.12',
  ...
  if (changed.length > 0) {
    ...
    console.log('  This may be from your edits, or from a open-ohcode update.\n');
    for (const file of changed) {
      console.log(`    ${file}`);
    }
    console.log('\n  To view differences:  npx open-ohcode diff <file>');
    console.log('  To reset to default:  npx open-ohcode reset <file>');
  }
  ...
  const open-ohcodePkg = JSON.parse(fs.readFileSync(path.join(packageDir, 'package.json'), 'utf8'));
  const version = open-ohcodePkg.version;

  if (!fs.existsSync(envPath)) {
    // Seed .env for new projects
    const authSecret = randomBytes(32).toString('base64');
    const seedEnv = `# open-ohcode Configuration
# Run "npm run setup" to complete configuration

AUTH_SECRET=${authSecret}
AUTH_TRUST_HOST=true
OPENOHCODE_VERSION=${version}
`;
    fs.writeFileSync(envPath, seedEnv);
    console.log(`  Created .env (AUTH_SECRET, OPENOHCODE_VERSION=${version})`);
  } else {
    // Update OPENOHCODE_VERSION in existing .env
    try {
      let envContent = fs.readFileSync(envPath, 'utf8');
      if (envContent.match(/^OPENOHCODE_VERSION=.*/m)) {
        envContent = envContent.replace(/^OPENOHCODE_VERSION=.*/m, `OPENOHCODE_VERSION=${version}`);
      } else {
        envContent = envContent.trimEnd() + `\nOPENOHCODE_VERSION=${version}\n`;
      }
      fs.writeFileSync(envPath, envContent);
      console.log(`  Updated OPENOHCODE_VERSION to ${version}`);
    } catch {}
  }
...
function reset(filePath) {
  ...
  if (!filePath) {
    ...
    console.log('\nUsage: open-ohcode reset <file>');
    console.log('Example: open-ohcode reset config/SOUL.md\n');
    return;
  }
  ...
  if (!fs.existsSync(src)) {
    ...
    console.log('Run "open-ohcode reset" to see available templates.\n');
    process.exit(1);
  }
...
function diff(filePath) {
  ...
  if (!filePath) {
    ...
    console.log('\nUsage: open-ohcode diff <file>');
    console.log('Example: open-ohcode diff config/SOUL.md\n');
    return;
  }
  ...
  if (!fs.existsSync(dest)) {
    ...
    console.log(`Run "open-ohcode reset ${filePath}" to create it.\n`);
    return;
  }
  ...
  try {
    ...
    console.log(`\n  To reset: open-ohcode reset ${filePath}\n`);
  }
}
...
async function setAgentSecret(key, value) {
  if (!key) {
    console.error('\n  Usage: open-ohcode set-agent-secret <KEY> [VALUE]\n');
    console.error('  Example: open-ohcode set-agent-secret ANTHROPIC_API_KEY\n');
    process.exit(1);
  }
...
async function setAgentLlmSecret(key, value) {
  if (!key) {
    console.error('\n  Usage: open-ohcode set-agent-llm-secret <KEY> [VALUE]\n');
    console.error('  Example: open-ohcode set-agent-llm-secret BRAVE_API_KEY\n');
    process.exit(1);
  }
...
async function setVar(key, value) {
  if (!key) {
    console.error('\n  Usage: open-ohcode set-var <KEY> [VALUE]\n');
    console.error('  Example: open-ohcode set-var LLM_MODEL claude-sonnet-4-5-20250929\n');
    process.exit(1);
  }
