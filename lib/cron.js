...
function getInstalledVersion() {
  const pkgPath = path.join(process.cwd(), 'node_modules', 'open-ohcode', 'package.json');
...
async function fetchAndStoreReleaseNotes(target) {
  try {
    const ghRes = await fetch(
      `https://api.github.com/repos/stephengpope/open-ohcode/releases/tags/v${target}`
    );
    if (!ghRes.ok) return;
...
/**
 * Check npm registry for a newer version of open-ohcode.
 */
  ...
  try {
    ...
    if (isPrerelease(installed)) {
      ...
      const results = await Promise.allSettled([
        fetch('https://registry.npmjs.org/open-ohcode/latest'),
        fetch('https://registry.npmjs.org/open-ohcode/beta'),
      ]);
      ...
      const res = await fetch('https://registry.npmjs.org/open-ohcode/latest');
      if (!res.ok) {
